...
export default async function Home() {
  ...
  const latestVideo = latestMatch ? findHighlightVideo(latestMatch, videos) : null;
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-8 transition-colors duration-300">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Spoiler-Free United</h1>
        ...
        {latestMatch ? (
          ...
          <div className="text-center py-10 bg-white dark:bg-gray-800 rounded-xl shadow-sm mb-10">
            <p className="text-gray-500 dark:text-gray-400">No recent matches found. Try refreshing the schedule.</p>
          </div>
        )}

        <div className="grid md:grid-cols-2 gap-8">
          <section>
            <h2 className="text-xl font-bold mb-4 text-gray-800 dark:text-gray-200">Recent Results</h2>
            <div className="space-y-4">
              {recentMatches.length > 0 ? (
                ...
                <p className="text-gray-500 dark:text-gray-400 italic">No other recent matches.</p>
              )}
            </div>
          </section>

          <section>
            <h2 className="text-xl font-bold mb-4 text-gray-800 dark:text-gray-200">Upcoming Fixtures</h2>
            <div className="space-y-4">
              {upcomingMatches.length > 0 ? (
                ...
                <p className="text-gray-500 dark:text-gray-400 italic">No upcoming fixtures scheduled.</p>
              )}
            </div>
          </section>
        </div>
