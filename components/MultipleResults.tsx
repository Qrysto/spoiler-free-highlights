...
export function MultipleResults({ videos }: MultipleResultsProps) {
  ...
  const increaseReveal = () => setRevealLength(prev => Math.min(prev + 10, 200));
  const decreaseReveal = () => setRevealLength(prev => Math.max(prev - 10, 10));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-white">
          Found {videos.length} possible highlight{videos.length !== 1 ? 's' : ''}
        </h3>
